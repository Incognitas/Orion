    ...
    loop {
        ...
        match current_opcode {
            ...
            bytecode::opcode_bspush => {
                execution_context.variables_stack.spush(
                        execution_context.bytecode_fetcher.fetch_b()? as i16
                    )?
                }
            // bytecode 17: SSPUSH
            bytecode::opcode_sspush => {
                execution_context.variables_stack.spush(
                        execution_context.bytecode_fetcher.fetch_s()? as i16
                    )? 
                }
            // bytecode 18: BIPUSH
            bytecode::opcode_bipush => {
                execution_context.variables_stack.ipush(
                        execution_context.bytecode_fetcher.fetch_b()? as i32
                    )? 
                }
            // bytecode 19: SIPUSH
            bytecode::opcode_sipush => {
                    execution_context.variables_stack.ipush(
                        execution_context.bytecode_fetcher.fetch_s()? as i32
                    )? 
                }
            // bytecode 20: IIPUSH
            bytecode::opcode_iipush => {
                    execution_context.variables_stack.ipush(
                        execution_context.bytecode_fetcher.fetch_i()? as i32
                    )?
                }
            // bytecode 21: ALOAD
            bytecode::opcode_aload => {
                ...
                let current_local = execution_context.current_frame()?.get_local_check_type(index, StackEntryType::reference)?;
            ...
            bytecode::opcode_sload => {
                let index = execution_context.bytecode_fetcher.fetch_b()?;
                // read local from current frame
                let current_local = execution_context.current_frame()?.get_local_check_type(index, StackEntryType::short)?;
            ...
            bytecode::opcode_iload => {
                ...
                // read local from current frame
                let current_local1 = execution_context.current_frame()?.get_local_check_type(index, StackEntryType::int)?;
                let current_local2 = execution_context.current_frame()?.get_local_check_type(index + 1, StackEntryType::int)?;
            ...
            bytecode::opcode_aload_3 => {
                ...
                let current_local = execution_context.current_frame()?
                                        .get_local_check_type(  current_opcode as u8 -
